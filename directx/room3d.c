...
// --- TCC互換性用定義 ---
// TCCのmath.hはfloat版関数が不足しているためマクロで対応
...
#define CLASS_NAME    "MAC_DX9_DEMO"
...
#define CUBE_SIZE   2.5f   // 中央キューブの大きさ
#define LIGHT_SIZE  1.0f   // 光源オブジェクトの大きさ
...
// テクスチャリソース
LPDIRECT3DTEXTURE9      g_pTexBrick    = NULL; // 壁用
LPDIRECT3DTEXTURE9      g_pTexConcrete = NULL; // 床・天井用
LPDIRECT3DTEXTURE9      g_pTexWood     = NULL; // 柱用
LPDIRECT3DTEXTURE9      g_pTexPlasma   = NULL; // キューブ用

// 描画管理（頂点バッファ内のオフセットとサイズ）
int g_TotalVertices = 0;
int g_RoomStartVert = 0;
int g_RoomVertCount = 0;
int g_PillarStartVert = 0;
int g_PillarVertCount = 0;
int g_CubeStartVert = 0;
int g_CubeVertCount = 0;
int g_LightObjStartVert = 0;
int g_LightObjVertCount = 0;

// カメラ・シーン状態
Vec3  g_CamPos = { 0.0f, 0.0f, -25.0f };    // カメラ位置
Vec3  g_LightPos = { 5.0f, 5.3f, -5.0f };   // 光源位置
float g_CameraYaw = 0.0f;                   // カメラ水平角度
float g_CameraPitch = 0.0f;                 // カメラ垂直角度
float g_CubeRotation = 0.0f;                // キューブ回転角
...
// テクスチャ生成
void CreateBrickTexture(void);
void CreateConcreteTexture(void);
void CreateWoodTexture(void);
void CreatePlasmaTexture(void);
...
HRESULT InitGeometry()
{
  ...
  if (FAILED(IDirect3DDevice9_CreateVertexBuffer(
      g_pd3dDevice,
      g_TotalVertices * sizeof(CUSTOMVERTEX),
      0,
      D3DFVF_CUSTOMVERTEX,
      D3DPOOL_DEFAULT,
      &g_pVB,
  ...
  if (FAILED(IDirect3DVertexBuffer9_Lock(g_pVB, 0, 0, (void**)&pVertices, 0)))
  {
    return E_FAIL;
  }
  ...
  CreatePlane(&pWalker, (Vec3){-W, H,  W}, (Vec3){ 2*W, 0, 0}, (Vec3){0, -2*H, 0}, (Vec3){ 0, 0, -1}, roomDiv, U_WALL, V_WALL); // 奥
  CreatePlane(&pWalker, (Vec3){ W, H, -W}, (Vec3){-2*W, 0, 0}, (Vec3){0, -2*H, 0}, (Vec3){ 0, 0,  1}, roomDiv, U_WALL, V_WALL); // 手前
  CreatePlane(&pWalker, (Vec3){-W, H, -W}, (Vec3){0, 0,  2*W}, (Vec3){0, -2*H, 0}, (Vec3){ 1, 0,  0}, roomDiv, U_WALL, V_WALL); // 左
  CreatePlane(&pWalker, (Vec3){ W, H,  W}, (Vec3){0, 0, -2*W}, (Vec3){0, -2*H, 0}, (Vec3){-1, 0,  0}, roomDiv, U_WALL, V_WALL); // 右
  CreatePlane(&pWalker, (Vec3){-W, H, -W}, (Vec3){ 2*W, 0, 0}, (Vec3){0, 0,  2*W}, (Vec3){ 0,-1,  0}, roomDiv, UV_FLOOR, UV_FLOOR); // 天井
  CreatePlane(&pWalker, (Vec3){-W,-H,  W}, (Vec3){ 2*W, 0, 0}, (Vec3){0, 0, -2*W}, (Vec3){ 0, 1,  0}, roomDiv, UV_FLOOR, UV_FLOOR); // 床

  // 柱生成
  float P = PILLAR_POS;
  float PS = PILLAR_SIZE;
  float PH = ROOM_HEIGHT * 2;
...
void UpdateInput(void)
{
  ...
  if (g_CamPos.x > L)  g_CamPos.x = L;
  if (g_CamPos.x < -L) g_CamPos.x = -L;
  if (g_CamPos.z > L)  g_CamPos.z = L;
  if (g_CamPos.z < -L) g_CamPos.z = -L;
}
...
void Render(void)
{
  ...
  if (SUCCEEDED(IDirect3DDevice9_BeginScene(g_pd3dDevice)))
  {
    ...
    // 強く発光させる (テクスチャなし)
    D3DMATERIAL9 mtrlLight;
    ZeroMemory(&mtrlLight, sizeof(mtrlLight));
...
void MatrixRotationYawPitchRoll(D3DMATRIX* out, float yaw, float pitch, float roll)
{
  ...
  out->m[0][0] = cy * cr + sy * sp * sr;
  out->m[0][1] = cp * sr;
  out->m[0][2] = -sy * cr + cy * sp * sr;

  out->m[1][0] = -cy * sr + sy * sp * cr;
  out->m[1][1] = cp * cr;
  out->m[1][2] = sr * sy + cy * sp * cr;
